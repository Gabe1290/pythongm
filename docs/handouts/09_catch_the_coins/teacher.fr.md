...
> TIP: **Projets de référence.** Téléchargez les projets par étapes sur le wiki (`09_catch_the_coins_checkpoints.zip`) : un projet pour la fin de chaque phase. Dans le projet de référence, `room_gameover` est une petite salle faite à la main, pas celle du modèle. Donnez à un élève bloqué la phase précédente.
