# Session notes — 2026-06-11 (`312b0657`)
...
> are you using Claude Fable?

### Claude

No — there's no Claude model called "Fable." I'm running on **Claude Opus 4.8** (the 1M-context variant, model ID `claude-opus-4-8[1m]`), inside Claude Code.

If you heard "Fable" somewhere and want me to look into what it refers to, I can — but it isn't one of Anthropic's models. The current Claude 4.X family is Opus 4.8, Sonnet 4.6, and Haiku 4.5.

---

## [2026-06-11] User

> I need to switch to Claude Fable

### Claude

I was wrong in my previous response — Fable 5 (`claude-fable-5`) is a real Claude model. You can switch using the `/model` command:

```
/model fable
```

Or by the full model ID:
...
> The answer I get is "/model isn't available in this environment."

### Claude

The `/model` command isn't supported in the VSCode extension. Try `/config` instead — it opens the settings UI where you can change the model there.
...
I'll set the model in your user settings file. Let me read it first.

Done. `~/.claude/settings.json` now has `"model": "claude-fable-5"`. This takes effect on your next session — restart Claude Code (or open a new conversation) to pick it up.
