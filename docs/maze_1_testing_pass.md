...
- [X] No `Qt Warning: QObject::connect(...)` lines (regression for
      commit `afb9c75`)
- [X] No duplicate `pygm.X` log lines (regression for commit `afb9c75`)
- [X] No `Unknown action type: X` warnings
- [X] No `No editor registered for asset type 'X'` warnings (rc.12
      covers rooms / objects / sprites / playgrounds / scripts)
- [X] No tracebacks, no `PermissionError`, no `FileNotFoundError`

## 2. Save behaviour (samples/ protection)

- [X] Make a small edit (e.g. rename an object, move an instance),
      press **Ctrl+S**. Save completes silently — no PermissionError,
      no "Refusing to save into bundled samples/" log line
- [X] After save, run `git status` in a separate shell —
      `samples/maze_1/` shows **no changes**
- [X] Window title shows `maze_1 — PyGameMaker IDE` (with `*` while
...
- [X] Expand **Sprites** in the asset tree (3 entries expected)
- [X] For each sprite: double-click → sprite editor opens →
      visually confirm:
      ...
      - Frame count is correct (1 frame for a static wall, multiple
        for an animation)
- [X] If you fix any sprite during this pass: edit happens against
...
- [X] Expand **Objects** in the asset tree (3 entries: typically
      something like `obj_player`, `obj_wall`, `obj_goal`)
- [X] Click each in turn; for each, the Properties panel on the
      right shows:
      ...
      - Any `Parent` field resolves to a real parent (likely blank
        for maze_1)
- [X] Double-click one of the objects with events (player movement,
...
- [X] Room editor: double-click both rooms. Each room opens, asset
      ...
      commit `00911b3`)
- [X] Instances render where they should on the canvas (player at
...
- [X] Toolbar **Test Game** (or **F5**) launches the game window
      without error
- [X] First room renders (player visible, walls visible, goal
      visible) — if you only see a blank or solid-colour window,
      that's an importer sprite-load failure
- [X] Player responds to input (likely arrow keys) — walks around
      the maze
- [X] Wall collision works — player cannot walk through walls
- [X] Reaching the goal triggers the room transition (advances
      to room 2 or shows a "you won" state — depends on what
      maze_1 was originally programmed to do)
- [X] Room 2 plays similarly — if maze_1 has a "win" or "loop"
      ending, that behaves correctly
- [X] Closing the game window returns control to the IDE cleanly,
      no traceback

## 7. Welcome tab inline list refresh

- [X] After working in maze_1 for a few minutes, close the project
      ...
      minutes ago")
- [X] The recent-projects list does **not** contain
