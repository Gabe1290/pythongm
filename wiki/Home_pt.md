...
![O IDE PyGameMaker com um projeto de plataformas aberto — árvore de recursos preenchida à esquerda, painel de propriedades à direita](images/ide-main-window.png)
...
|--------------|------------|-----------------|
| [**Iniciante**](Beginner-Preset_pt) | Novos em desenvolvimento de jogos | O essencial — movimento, colisões, pontuação, salas |
| [**Intermediário**](Intermediate-Preset_pt) | Alguma experiência | Adiciona vidas, saúde, som, alarmes, desenho |
| **Avançado (Completo)** | Usuários experientes | Tudo — os 37 eventos e 109 ações |
...
| **Vista 2.5D / Primeira pessoa** | Renderização raycast opcional no estilo Doom/Wolfenstein — veja [Vista 3D](3D-View_pt) |
| **Exportação Multi-Plataforma** | Windows EXE, app macOS, HTML5, Linux, Kivy (móvel/desktop) |
...
| **Extensível** | Sistema de [extensões](Extensions_pt) e plugins para ações e renderizadores personalizados |
...
### Ações
Operações realizadas quando eventos são disparados. **109** ações integradas para:
- Movimento e física
- Desenho e sprites
...
### App macOS
Pacotes `.app` nativos para macOS via PyInstaller.
...
- [Editor de Código](Code-Editor_pt) - Escrever (e gerar) Python real para um objeto
...
- [Gestor de Recursos](Asset-Manager_pt) - Rastreio de utilização, Lixo, e procura de ficheiros não utilizados/órfãos
...
A documentação do PyGameMaker está dividida em três lugares, cada um adequado a um tipo de pergunta diferente:

| Se você quer... | Consulte aqui |
|---|---|
| Aprofundar um conceito ou funcionalidade (editores, eventos, ações, exportação) | **Este wiki** |
| Seguir uma lição guiada, clique a clique, dentro do aplicativo | O painel **Tutoriais** do IDE (`Ajuda > Tutoriais`) — um currículo de 9 lições, do primeiro projeto até um jogo completo com vitória/derrota |
| Entender um projeto de exemplo específico (`samples/`) incluído com o PyGameMaker | O **guia** próprio desse exemplo — clique em **📖 Sample guides** na aba de Boas-vindas *(botão ainda não traduzido para o português — ver `TODO.md`)*; cada exemplo inclui um `README.md` (traduzido por idioma) escrito especificamente para ele |
...
O código-fonte do PyGameMaker é licenciado sob a **Licença MIT**.  Esta documentação é licenciada sob **CC BY 4.0**.
