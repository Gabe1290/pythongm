...
Esta página lista todas as **113** ações disponíveis no PyGameMaker, exatamente como aparecem no seletor de ações do IDE (incluindo o plugin Audio e a extensão Vista 3D). Ações são comandos que são executados quando um evento é acionado.
