"""Regression test: ru's 153 previously-unfinished translations.

Part of closing the gap docs/I18N_UNFINISHED_2026-08-10.md — fourth
executed slice (unit 4/7), after fr, de, and it. Covers the shared
~148-string gap common to de/it/ru/sl/uk (see the plan doc) plus 2
ru-specific entries beyond that shared set (BackgroundLayersDialog's
"Visible:" and "None", already translated in the other four languages),
routed across ru's 6 split translation files. Several strings turned out
to already have a real Russian translation sitting in a SISTER split
file under the same (context, source) pair (BlocklyVisualProgrammingTab
/ BlocklyWidget duplicated across blockly.ts and misc.ts;
VisualScriptingArea / ActionListWidget duplicated across actions.ts and
editors.ts) — those were reused verbatim rather than re-translated, for
wording consistency.
...
FILES = {
    "core": TRANS_DIR / "pygm2_ru_core.ts",
    "editors": TRANS_DIR / "pygm2_ru_editors.ts",
    "actions": TRANS_DIR / "pygm2_ru_actions.ts",
    "blockly": TRANS_DIR / "pygm2_ru_blockly.ts",
    "dialogs": TRANS_DIR / "pygm2_ru_dialogs.ts",
    "misc": TRANS_DIR / "pygm2_ru_misc.ts",
}
...
def test_no_double_escaped_translations():
    ...
    (fixed separately) -- guard the ru batch against the same mistake."""
    for path in FILES.values():
        content = path.read_text(encoding="utf-8")
...
def test_runtime_translate_resolves_every_string():
    ...
    QCoreApplication.translate (same landmine as the fr/de/it batches)."""
