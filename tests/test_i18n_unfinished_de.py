"""Regression test: de's 151 previously-unfinished translations.

Part of closing the gap docs/I18N_UNFINISHED_2026-08-10.md — the original
finding (this is the language that surfaced the gap, via the Preferences
dialog's "General" tab rendering in English during the extension-UI fix's
screenshot spike). Second executed slice after fr; covers the shared
~148-string gap common to de/it/ru/sl/uk (see the plan doc), routed
across de's 6 split translation files.
...
FILES = {
    "core": TRANS_DIR / "pygm2_de_core.ts",
    "editors": TRANS_DIR / "pygm2_de_editors.ts",
    "actions": TRANS_DIR / "pygm2_de_actions.ts",
    "blockly": TRANS_DIR / "pygm2_de_blockly.ts",
    "dialogs": TRANS_DIR / "pygm2_de_dialogs.ts",
    "misc": TRANS_DIR / "pygm2_de_misc.ts",
}
...
# "Edition" is a genuine German cognate (confirmed by an offscreen
# screenshot spike of the Preferences dialog: "Edition:" reads correctly
# in German as-is) -- not a stray untranslated copy. "Open Roberta XML
# (*.xml)" is a deliberately-untranslated file-dialog filter: "Open
# Roberta" is the proper name of the XML format, and "(*.xml)" is a
# literal glob pattern -- nothing in the string is real German prose.
_COGNATES = {
    ("core", "PreferencesDialog", "Edition:"),
    ("core", "PyGameMakerIDE", "Open Roberta XML (*.xml)"),
}
...
def test_runtime_translate_resolves_every_string():
    ...
    QCoreApplication.translate (same landmine as the fr batch)."""
