...
def _stub_with_open_editor(qapp, category, asset_name, dirty=True):
    ...
    # open_editors is keyed by the composite "<category>:<name>" (audit L5),
    # so register the existing editor under that key and give the stub the
    # real key helpers the open_*_editor methods call on self.
    key = PyGameMakerIDE._editor_key(category, asset_name)
    stub = SimpleNamespace(
        open_editors={key: existing},
        editor_tabs=tabs,
        _focus_detached_editor=lambda k: False,
        _editor_key=PyGameMakerIDE._editor_key,
