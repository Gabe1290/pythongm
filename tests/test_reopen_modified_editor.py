...
def _stub_with_open_editor(qapp, category, asset_name, dirty=True):
    ...
    # open_editors is keyed by the composite "<category>:<name>" key (L5).
    stub = SimpleNamespace(
        open_editors={f"{category}:{asset_name}": existing},
        editor_tabs=tabs,
        _focus_detached_editor=lambda key: False,
        # Behaves like the bound IDE helper; categories here are already canonical.
