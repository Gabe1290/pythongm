"""Regression test: sl's 153 previously-unfinished translations.

Part of closing the gap docs/I18N_UNFINISHED_2026-08-10.md — fifth
executed slice (unit 5/7), after fr, de, it, and ru. Covers the shared
~148-string gap common to de/it/ru/sl/uk (see the plan doc) plus 2
entries beyond that shared set (BackgroundLayersDialog's "Visible:" and
"None", also present in ru but not in it/de), routed across sl's 6
split translation files -- the exact same 153 (group, context, source)
triples as ru's own batch (verified by direct diff before translating).
Several strings turned out to already have a real Slovenian translation
sitting in a SISTER split file under the same (context, source) pair
(BlocklyVisualProgrammingTab / BlocklyWidget duplicated across
blockly.ts and misc.ts; VisualScriptingArea / ActionListWidget
duplicated across actions.ts and editors.ts) — those were reused
verbatim rather than re-translated, for wording consistency.
...
FILES = {
    "core": TRANS_DIR / "pygm2_sl_core.ts",
    "editors": TRANS_DIR / "pygm2_sl_editors.ts",
    "actions": TRANS_DIR / "pygm2_sl_actions.ts",
    "blockly": TRANS_DIR / "pygm2_sl_blockly.ts",
    "dialogs": TRANS_DIR / "pygm2_sl_dialogs.ts",
    "misc": TRANS_DIR / "pygm2_sl_misc.ts",
}

# (group, context, source) for all 153 strings this batch filled in.
SOURCES = [
    ...
    ("editors", "BackgroundLayersDialog", "Visible:"),
    ("editors", "BackgroundLayersDialog", "Foreground:"),
    ("editors", "BackgroundLayersDialog", "None"),
...
assert len(SOURCES) == 153, len(SOURCES)

# "Open Roberta XML (*.xml)" is a deliberately-untranslated file-dialog
# filter (proper name + literal glob pattern), same exception fr/de/it made.
_COGNATES = {
    ("core", "PyGameMakerIDE", "Open Roberta XML (*.xml)"),
}
...
def test_no_double_escaped_translations():
    ...
    (fixed separately) -- guard the sl batch against the same mistake."""
    for path in FILES.values():
        content = path.read_text(encoding="utf-8")
...
def test_runtime_translate_resolves_every_string():
    ...
    QCoreApplication.translate (same landmine as the fr/de/it/ru batches)."""
