"""Regression test: uk's 152 previously-unfinished translations.

Part of closing the gap docs/I18N_UNFINISHED_2026-08-10.md — sixth
executed slice (unit 6/7), after fr, de, it, ru, and sl. Covers the
shared ~148-string gap common to de/it/ru/sl/uk (see the plan doc) plus
1 entry beyond that shared set (BackgroundLayersDialog's "None"; uk
already had "Visible:" translated, unlike ru/sl). Verified by direct
diff against ru's batch before translating: uk's unfinished set is
identical to ru's 153 minus "Visible:". Several strings turned out to
already have a real Ukrainian translation sitting in a SISTER split
...
FILES = {
    "core": TRANS_DIR / "pygm2_uk_core.ts",
    "editors": TRANS_DIR / "pygm2_uk_editors.ts",
    "actions": TRANS_DIR / "pygm2_uk_actions.ts",
    "blockly": TRANS_DIR / "pygm2_uk_blockly.ts",
    "dialogs": TRANS_DIR / "pygm2_uk_dialogs.ts",
    "misc": TRANS_DIR / "pygm2_uk_misc.ts",
}

# (group, context, source) for all 152 strings this batch filled in.
SOURCES = [
...
# filter (proper name + literal glob pattern), same exception fr/de/it/ru/sl made.
_COGNATES = {
    ("core", "PyGameMakerIDE", "Open Roberta XML (*.xml)"),
}
...
def test_no_double_escaped_translations():
    ...
    (fixed separately) -- guard the uk batch against the same mistake."""
    for path in FILES.values():
        content = path.read_text(encoding="utf-8")
...
def test_runtime_translate_resolves_every_string():
    ...
    QCoreApplication.translate (same landmine as the fr/de/it/ru/sl batches)."""
