...
class ActionConverter:
    ...
    def _convert_if_on_grid(self, params: Dict[str, Any], indent_level: int) -> str:
        ...
        code_lines.append(self._indent("_nearest_x = round(self.x / _grid) * _grid", indent_level))
        code_lines.append(self._indent("_nearest_y = round(self.y / _grid) * _grid", indent_level))
        code_lines.append(self._indent("_on_grid = abs(self.x - _nearest_x) < 2 and abs(self.y - _nearest_y) < 2", indent_level))
        code_lines.append(self._indent("if _on_grid:", indent_level))
    ...
    def _convert_set_hspeed_smooth(self, params: Dict[str, Any], indent_level: int) -> str:
        ...
        speed = float(speed)
        code_lines = []
        code_lines.append(self._indent("# Smooth movement - instant direction change", indent_level))
