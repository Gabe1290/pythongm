...
class GameApp(App):
    ...
    def build(self):
        ...
        # Schedule game loop
        Clock.schedule_interval(self.scene.update, 1.0 / {settings.get('room_speed', 60)})
...
class GameObject(Widget):
    ...
    def _process_movement(self, dt):
        """Process movement (called each frame)"""
        # Note: This is called from update which gets dt, but movement happens at fixed rate
        # For smooth movement at 60 FPS, speeds are in pixels per frame
        ...
        # Update position with float precision for smooth movement
        new_x = self.x + float(self.hspeed * dt * 60)
        new_y = self.y + float(self.vspeed * dt * 60)
        ...
        if self.solid:
            ...
            if not can_move:
                ...
                # Snap to grid if close (for smooth movement with grid logic)
                self.snap_to_grid_if_close()
    ...
    def is_on_grid(self, tolerance=0.5):
        """Check if object is aligned to grid (with tolerance for floating point)"""
        x_remainder = self.x % self.grid_size
        y_remainder = self.y % self.grid_size
        
        # Check if we're within tolerance of grid alignment
        x_aligned = (x_remainder < tolerance or x_remainder > self.grid_size - tolerance)
        y_aligned = (y_remainder < tolerance or y_remainder > self.grid_size - tolerance)
        
        return x_aligned and y_aligned
    
    def snap_to_grid_if_close(self, tolerance=0.5):
        """Snap to grid if close enough (for smooth movement with grid logic)"""
        x_remainder = self.x % self.grid_size
        y_remainder = self.y % self.grid_size
        
        # Snap X if close to grid
        if x_remainder < tolerance:
            self.x = float(int(self.x / self.grid_size) * self.grid_size)
        elif x_remainder > self.grid_size - tolerance:
            self.x = float((int(self.x / self.grid_size) + 1) * self.grid_size)
        
        # Snap Y if close to grid
        if y_remainder < tolerance:
            self.y = float(int(self.y / self.grid_size) * self.grid_size)
        elif y_remainder > self.grid_size - tolerance:
            self.y = float((int(self.y / self.grid_size) + 1) * self.grid_size)
    ...
    def snap_to_grid(self):
        ...
        self.x = float(round(self.x / self.grid_size) * self.grid_size)
        self.y = float(round(self.y / self.grid_size) * self.grid_size)
        self._update_position()
