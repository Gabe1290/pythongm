...
class MacOSExporter(BaseKivyExporter):
...
'''
    ...
    def _create_spec_file(self, build_dir: Path, launcher_script: Path) -> Path:
        ...
        # PyInstaller cannot detect Kivy dependencies through static
        # analysis.  We must list every dynamically-loaded provider
        # and UIX module explicitly.
        hidden_imports = [
            'kivy',
            'kivy.app',
            'kivy.base',
...
_kivy_pkg = os.path.dirname(_kivy.__file__)
...
a = Analysis(
    ...
    datas=[
        {datas_str},
        (os.path.join(_kivy_pkg, 'data'), 'kivy/data'),
    ],
    hiddenimports=[
        {hidden_imports_str}
    ],
...
'''
    ...
    def _copytree_resolve_symlinks(self, src: Path, dst: Path):
        """Copy a directory tree, resolving symlinks to actual files/dirs.

        Unlike shutil.copytree(symlinks=False), this correctly handles
        symlinks that point to directories by copying the resolved directory
        contents instead of failing with EISDIR (Errno 21).
        """
        ...
        for item in src.iterdir():
            ...
            try:
                if item.is_symlink():
                    ...
                    if target.is_dir():
                        if not d.exists():
                            self._copytree_resolve_symlinks(target, d)
                    else:
                        shutil.copy2(target, d)
                elif item.is_dir():
                    self._copytree_resolve_symlinks(item, d)
                else:
                    shutil.copy2(item, d)
            except Exception as e:
                logger.debug("Skipped {}: {}".format(item.name, e))
    ...
    def _copy_to_output(self, build_dir: Path):
        ...
        by resolving symlinks to actual directory copies.
        """
        dist_dir = build_dir / "dist"
            ...
            for item in dist_dir.iterdir():
                ...
                if item.is_dir():
                    ...
                    try:
                        ...
                        # Destination doesn't support symlinks (ExFAT/FAT32).
                        # Use custom copy that resolves symlinks to real
                        # files/directories instead of failing.
                        logger.info("Destination does not support symlinks, "
                                    "resolving symlinks to copies")
                        if dest.exists():
                            shutil.rmtree(dest)
