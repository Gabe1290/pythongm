...
class WSLBridge:
    ...
    def run_buildozer(self, build_dir_windows: str) -> subprocess.Popen:
        ...
        build_script = (
            ...
            '\n'
            '# Ensure build-tools 33.0.2 is installed (has standalone aidl).\n'
            '# Newer build-tools (34+) removed aidl which breaks buildozer.\n'
            'SDK_ROOT="$HOME/.buildozer/android/platform/android-sdk"\n'
            'if [ ! -f "$SDK_ROOT/build-tools/33.0.2/aidl" ]; then\n'
            '    echo "Installing Android build-tools 33.0.2..."\n'
            '    SDKMGR=$(find "$SDK_ROOT" -name sdkmanager 2>/dev/null | head -1)\n'
            '    if [ -n "$SDKMGR" ]; then\n'
            '        yes | "$SDKMGR" --sdk_root="$SDK_ROOT" '
            '"build-tools;33.0.2" > /dev/null 2>&1 || true\n'
            '    fi\n'
            'fi\n'
            '\n'
