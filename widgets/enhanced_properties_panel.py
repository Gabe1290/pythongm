...
class EnhancedPropertiesPanel(QWidget):
        ...
        try:
            ...
            while parent:
                if hasattr(parent, 'current_project_data') and parent.current_project_data:
                    ...
                    # Combine backgrounds and sprites
                    backgrounds = assets.get('backgrounds', {}).copy()
                    backgrounds.update(assets.get('sprites', {}))
                    return backgrounds
                parent = parent.parent()
            return {}
    ...
    def set_asset(self, asset_data):
        ...
        # If we're in room editor or object editor context, don't override with asset tree selections
        if self.current_room_editor or self.current_object_editor:
            return
    ...
    def show_asset_properties(self, asset_type: str, asset_name: str, asset_data: Dict[str, Any]):
        ...
        if asset_type == 'rooms':
            ...
            self.properties_layout.addRow("Type:", QLineEdit("Background"))
    ...
    def show_object_properties(self, object_data: Dict[str, Any]):
        ...
        try:
            ...
            # Connect signal AFTER setting initial value to prevent false triggers
            ...
            self.visible_check.toggled.connect(lambda v: self._on_property_changed('visible', v))
            
            self.solid_check = QCheckBox()
            self.solid_check.setChecked(object_data.get('solid', False))
            self.solid_check.toggled.connect(lambda v: self._on_property_changed('solid', v))
            
            self.persistent_check = QCheckBox()
            self.persistent_check.setChecked(object_data.get('persistent', False))
            self.persistent_check.toggled.connect(lambda v: self._on_property_changed('persistent', v))
            ...
            QTimer.singleShot(200, self._clear_updating_flag)

    def _clear_updating_flag(self):
        """Clear the updating properties flag"""
        self._updating_properties = False

    def _on_sprite_changed(self, sprite_name):
        """Handle sprite combo changes with recursion prevention"""
        if getattr(self, '_updating_properties', False):
            return
        
        value = sprite_name if sprite_name != "None" else ''
        self._on_property_changed('sprite', value)

    def _on_property_changed(self, property_name: str, value):
        """Internal property change handler with recursion prevention"""
        if getattr(self, '_updating_properties', False):
            return
            
        self.on_object_property_changed(property_name, value)
    ...
    def show_object_preview(self, object_data: Dict[str, Any]):
        ...
        if sprite_name and hasattr(self, 'current_object_editor'):
            # Try to load sprite preview
            available_sprites = getattr(self.current_object_editor, 'available_sprites', {})
