...
class AssetTreeWidget(QTreeWidget):
    ...
    def create_separator_item(self):
        ...
        separator.setBackground(0, self.palette().color(self.palette().ColorRole.Mid))
    ...
    def show_context_menu(self, position):
        ...
        if not item or not isinstance(item, AssetTreeItem):
            return
        
        if item.is_category:
            return  # Don't show menu for categories
        
        # Create context menu
        context_menu = QMenu(self)
        
        # Open action (always first)
        ...
        if item.asset_type in ['objects', 'rooms']:
            export_action = QAction(f"📦 Export {item.asset_type[:-1].title()}...", self)
            export_action.triggered.connect(lambda: self.export_resource(item.asset_type, item.asset_name))
        ...
        # Add room ordering options if this is a room
        if item.asset_type == "rooms":
            context_menu.addSeparator()
            
            # Get room list to check position
            room_list = self.get_room_list()
            room_index = room_list.index(item.asset_name) if item.asset_name in room_list else -1
            
            # Move Up action (disabled if first)
            move_up_action = QAction("⬆️ Move Up", self)
            move_up_action.setEnabled(room_index > 0)
            move_up_action.triggered.connect(lambda: self.move_room_up(item.asset_name))
            context_menu.addAction(move_up_action)
            
            # Move Down action (disabled if last)
            move_down_action = QAction("⬇️ Move Down", self)
            move_down_action.setEnabled(0 <= room_index < len(room_list) - 1)
            ...
            move_top_action.setEnabled(room_index > 0)
            ...
            move_bottom_action.setEnabled(0 <= room_index < len(room_list) - 1)
