...
PAGE_CHROME = {
    ...
    "uk": {
        "beginner_title": "Пресет для Початківців",
        "intermediate_title": "Середній Пресет",
        "nav": "*[Головна](Home_uk) | [Посібник з Пресетів](Preset-Guide_uk) | {other_link}*",
        "autogen": "> **Автоматично згенеровано** з `get_{preset}()` у "
                   "`config/blockly_config.py` за допомогою "
                   "`tools/gen_preset_docs.py` — не редагуйте вручну; "
                   "запустіть генератор знову після зміни пресету.",
        "scope_note": "> **Що цей пресет насправді обмежує:** цей пресет "
                       "фільтрує ОДНОЧАСНО візуальну палітру блоків Blockly "
                       "ТА меню «Додати подію»/«Додати дію» структурованої "
                       "панелі Події/Дії — незалежно від того, який редактор "
                       "ви використовуєте, з'являються лише події/дії, "
                       "перелічені нижче. Пресет *проєкту* встановлюється "
                       "двома способами: **`Налаштування > IDE Edition`** "
                       "вибирає типовий пресет для *нових* проєктів (видання "
                       "Початківець -> цей пресет; наявні проєкти ніколи не "
                       "змінюються при перемиканні видання), а **`Інструменти "
                       "> Налаштувати блоки дій...`** змінює пресет "
                       "*поточно відкритого* проєкту в будь-який момент. "
                       "Типове видання IDE — Початківець, тому нові проєкти "
                       "чистої інсталяції починаються саме з цього списку.",
        "overview": "Огляд",
        "overview_text": "Цей пресет вмикає **{n_events}** типів подій і "
                          "**{n_actions}** типів дій.",
        "events_h": "Події",
        "actions_h": "Дії",
        "c_event": "Подія", "c_block": "Назва Блоку", "c_category": "Категорія", "c_desc": "Опис",
        "c_action": "Дія", "c_params": "Параметри",
        "none": "—",
        "see_also": "Дивіться Також",
        "sa_preset": "- [Посібник з Пресетів](Preset-Guide_uk) — що таке пресети і як їх змінити",
        "sa_events": "- [Довідник Подій](Event-Reference_uk) — повний опис кожної події",
        "sa_actions": "- [Повний Довідник Дій](Full-Action-Reference_uk) — повні деталі параметрів для кожної дії",
        "sa_other": "- [{other_title}]({other_link}) — {other_desc}",
        "intermediate_desc": "наступний рівень",
        "beginner_desc": "рівень нижче цього",
    },
    "ru": {
        "beginner_title": "Пресет для Начинающих",
        "intermediate_title": "Средний Пресет",
        "nav": "*[Главная](Home_ru) | [Руководство по Пресетам](Preset-Guide_ru) | {other_link}*",
        "autogen": "> **Автоматически сгенерировано** из `get_{preset}()` в "
                   "`config/blockly_config.py` с помощью "
                   "`tools/gen_preset_docs.py` — не редактируйте вручную; "
                   "запустите генератор заново после изменения пресета.",
        "scope_note": "> **Что этот пресет на самом деле ограничивает:** этот "
                       "пресет фильтрует ОДНОВРЕМЕННО визуальную палитру "
                       "блоков Blockly И меню «Добавить событие»/«Добавить "
                       "действие» структурированной панели События/Действия — "
                       "независимо от того, каким редактором вы пользуетесь, "
                       "появляются только события/действия, перечисленные "
                       "ниже. Пресет *проекта* задаётся двумя способами: "
                       "**`Настройки > IDE Edition`** выбирает пресет по "
                       "умолчанию для *новых* проектов (издание Начинающий "
                       "-> этот пресет; существующие проекты никогда не "
                       "меняются при смене издания), а **`Инструменты > "
                       "Настроить блоки действий...`** меняет пресет "
                       "*текущего открытого* проекта в любой момент. "
                       "Издание IDE по умолчанию — Начинающий, поэтому "
                       "новые проекты чистой установки начинаются именно с "
                       "этого списка.",
        "overview": "Обзор",
        "overview_text": "Этот пресет включает **{n_events}** типов событий и "
                          "**{n_actions}** типов действий.",
        "events_h": "События",
        "actions_h": "Действия",
        "c_event": "Событие", "c_block": "Имя Блока", "c_category": "Категория", "c_desc": "Описание",
        "c_action": "Действие", "c_params": "Параметры",
        "none": "—",
        "see_also": "Смотрите Также",
        "sa_preset": "- [Руководство по Пресетам](Preset-Guide_ru) — что такое пресеты и как их изменить",
        "sa_events": "- [Справочник Событий](Event-Reference_ru) — полное описание каждого события",
        "sa_actions": "- [Полный Справочник Действий](Full-Action-Reference_ru) — полные сведения о параметрах для каждого действия",
        "sa_other": "- [{other_title}]({other_link}) — {other_desc}",
        "intermediate_desc": "следующий уровень",
        "beginner_desc": "уровень ниже этого",
    },
    "fr": {
